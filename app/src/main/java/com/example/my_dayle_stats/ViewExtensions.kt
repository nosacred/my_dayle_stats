...
fun getCompostiteStatList(orders : List<Order>, sales : List<Sale>, stocks: List<Stock>) : List<CompositeStat>{
...
//    barcodes.addAll(stocks.stream().map(Stock::barcode).collect(Collectors.toList()))
    ...
    for(bcode in barcodes){
        ...
        whSet.addAll(stocks.stream().filter{it.barcode==bcode}.map { it.warehouseName }.collect(Collectors.toList()))
        whSet.sorted()
        ...
        val inWayToClient =stocks.stream().filter { it.barcode==bcode }.mapToInt { it.inWayToClient }.sum()
        val inWayFromClient =stocks.stream().filter { it.barcode==bcode }.mapToInt { it.inWayFromClient }.sum()
        ...
        regionNameList.addAll(orders.stream().filter { it.barcode==bcode }.map { it.regionName }.collect(Collectors.toList()))
        regionNameList.addAll(sales.stream().filter { it.barcode==bcode }.map { it.regionName }.collect(Collectors.toList()))
        val cancelCountOrder=orders.stream().filter { it.barcode==bcode }.filter{it.isCancel}.count().toInt()
        val cancelCountSale=sales.stream().filter { it.barcode==bcode }.filter{it.forPay < 0}.count().toInt()
        ...
        val totalOrdersPrice=orders.stream().filter { it.barcode==bcode }.mapToInt { it.finishedPrice.toInt()}.sum()
        val totalSaleForPay=sales.stream().filter { it.barcode==bcode }.filter { it.forPay<0 }.mapToInt { it.forPay.toInt()}.sum()
        val moneyBack=sales.stream().filter { it.barcode==bcode }.mapToInt { it.forPay.toInt()}.sum()



        val compositeStat = CompositeStat(whSet.toList(), supplierArticle, nmId, bcode, quantity, inWayToClient, inWayFromClient, category, subject, brand, techSize,
            regionNameList.toList(), cancelCountOrder,cancelCountSale,countSale,countOrder,totalOrdersPrice,totalSaleForPay,moneyBack)
...
fun getAbcClassList(sales : List<Sale>): List<AbClass>{
    ...
    for(bc in bcodes){
        ...
        val abc  = AbClass(
            ...
            cancelCountSale = sales.filter { it.barcode == bc }.filter{ it.forPay.toInt() <0 }.count(),
            ...
            moneyBack = sales.filter { it.barcode == bc }.filter{ it.forPay.toInt() <0 }.map { it.forPay.toInt() }.sum(),
...
fun getAbcCList(sales : List<Sale>): List<AbClass>{
    ...
    hashMapC = hashMap.toList().sortedBy { (key,value)-> value }.reversed().toMap() as HashMap<String, Int>
