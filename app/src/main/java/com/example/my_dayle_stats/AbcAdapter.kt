...
class AbcAdapter(private val abc_item : List<AbClass>):RecyclerView.Adapter<AbcAdapter.Holder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AbcAdapter.Holder {
        return AbcAdapter.Holder(
            parent.inflate(R.layout.items_abc),
        )
    }

    @RequiresApi(Build.VERSION_CODES.P)
    override fun onBindViewHolder(holder: AbcAdapter.Holder, position: Int) {
    ...
    class Holder(
        ...
        @RequiresApi(Build.VERSION_CODES.P)
        private val i_abc_subject_category =view.requireViewById<TextView>(R.id.i_abc_subject_category)
        @RequiresApi(Build.VERSION_CODES.P)
